...
st.set_page_config(page_title="Car Price Predicition",page_icon=":car:",layout="centered",initial_sidebar_state="auto")
import numpy as np
import pandas as pd
word_int={"two":2,"three":3,"four":4,"five":5,"six":6,"eight":8,"twelve":12}
def num_map(ser):
  return ser.map(word_int)
@st.cache
def load_data():
...
inp=st.sidebar.radio("Go to",tuple(pages_dict.keys))
if inp=="Home":
  home.app()
else:
  v=pages_dict[inp]
